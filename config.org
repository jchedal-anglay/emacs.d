...
(defun ign:apply-formatter (command &rest args)
  (apply #'ign:sh-run-safe command (nconc args (list (buffer-file-name))))
  (ign:refresh-buffer))
#+END_SRC
...
(use-package tuareg
  ...
  :after company
  :preface
  (defun ocaml-before-save-hook ()
    (when (eq major-mode 'tuareg-mode)
      (ign:apply-formatter "ocamlformat" "-i")))
  :mode ("\\.mly\\'" . tuareg-menhir-mode)
  :custom
  (tuareg-match-patterns-aligned t)
  (tuareg-indent-align-with-first-arg t)
  :config
  (add-hook 'before-save-hook #'ocaml-before-save-hook))
