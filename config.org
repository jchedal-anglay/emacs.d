...
#+begin_src emacs-lisp
  ...
  (use-package god-mode
    :after (key-chord which-key)
    :chords
    (("th" . god-mode-all))
    :custom
    (god-mod-alist '((nil . "C-") ("SPC" . "M-")))
    (god-literal-key "u")
    :config
    (cl-defun ign//swap-x-t ((key))
      "Swap ?x and ?t KEY arguments for god-mode."
      (pcase key
        (?x `(?t))
        (?t `(?x))
        (_ `(,key))))
    (advice-add 'god-mode-sanitized-key-string :filter-args #'ign//swap-x-t)
